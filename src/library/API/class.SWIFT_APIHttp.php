...
class SWIFT_APIHttp extends SWIFT_HttpCurl
{
	...
	public function __construct()
	{
		...
		$this->_defaultReqOpts = array(CURLOPT_MAXCONNECTS => $_maxConnect, CURLOPT_CONNECTTIMEOUT => $_connectTimeout,
			CURLOPT_TIMEOUT => $_timeout, CURLOPT_FOLLOWLOCATION => $_redirection);
	 ...
	 *
	 * @author Atul Atri
	 * @param string $_bcAppName applciation name
	 * @param string $_bcAppEmail
	 *
	 * @throws SWIFT_API_Exception application  name or applciation email is email
	 */
	public function SetDefaultUserAgent($_bcAppName, $_bcAppEmail)
	{
		if (empty($_bcAppName) || !filter_var($_bcAppEmail, FILTER_VALIDATE_EMAIL)) {
			throw new SWIFT_API_Exception('Invalid application name or email');
		}
